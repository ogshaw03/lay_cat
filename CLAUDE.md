...
### パッチノートの文体・粒度
- **とにかくシンプル**にする。ユーザー向けに「何が変わったか」だけを短く。
- 各機能ブロックは **見出し（絵文字＋一言タイトル）＋ 1〜2 行の要約** が基本。細かい実装解説（内部処理・CSS 変数・具体的な閾値など）は書かない。
- 必要なら追加箇条書きも短く 1 行ずつ。長い bullet ネストや実装詳細（内部で HTML5 D&D をやめて Pointer Events にした 等）は載せない。
- **「刷新」という言葉は分かりづらいので使わない**。「作り直し」「改良」「見直し」「変更」など平易な語で置き換える。
- 詳細は `UPDATE_LOG.md` に既に記録してあるので、パッチノートで重複させない。
