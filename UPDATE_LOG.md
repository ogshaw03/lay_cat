...
- 「チェック依頼」通知がチェック担当者に届かない不具合を修正。判定を 2 段階で強化：(1) サブミット作成時に `byEmail`（認証メール）を保存し「自分の提出をスキップ」判定をメールベースで厳密化（同姓同名対策・従来は表示名だけの比較で誤スキップの余地があった）。あわせて担当者名の比較を大文字小文字無視化。(2) 重複通知抑止を「`localStorage` の seen キー」から「`DB.notifications` に該当キーが既にあるか」へ切替。従来は seen キーだけが残って `DB.notifications` への push が persist 失敗等で消えた場合に一度スキップされ始めると恒久的に静音化していた。通知本体に `rqKey` を付与して同一チェック依頼の再通知を DB 側で判定する方式に変更。過去に残った seen キーは、対応する DB 通知が無い場合に自動で削除して復活を許可。
