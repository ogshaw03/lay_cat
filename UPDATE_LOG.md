...
- (dev v2026.07.28.003) 投げ縄アイコンの尻尾を短くして輪とのバランス調整
  - 縄の端の垂れる長さが輪に対して長すぎたため、終端座標を `(4,20)` → `(5.5,17)` に短縮。輪の比率が上がって全体のシルエットがまとまる。

- (dev v2026.07.28.002) 投げ縄ツールのアイコンを Photoshop 風ロープループに差し替え
  - 従来は破線の楕円＋小さな尻尾で意味が伝わりにくかったため、実線のロープが輪を作って縄の端が垂れて先端が丸まる（Photoshop 準拠）デザインに変更。
  - `LASSO_SVG` のみ変更、ロジックは無変更。

- (dev v2026.07.28.001) 3D エディタのプリセット（T ポーズ／A ポーズ／立ち／座り／走り／手を振る）UI を撤去
  - サイドバーの `<h3>プリセット</h3>` セクションと `.presetGrid` の HTML／CSS を削除。「初期ポーズ」ボタン（ヘッダ）は残置（内部で `applyPreset('tpose')` を呼び bind pose にリセット）。
  - `PRESETS` オブジェクト・`applyPreset` 関数は「初期ポーズ」用に温存。
  - beta 側は `mannequin_3d.html`（dev/beta 共有ファイル）から自動的に反映されるが、ブラウザキャッシュ上は `?v=beta v0.0.6` のままなので、beta ユーザーは Ctrl+Shift+R で強制リロードするか次の beta 反映まで待つ必要あり。
