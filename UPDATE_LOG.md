...
- (dev v2026.08.07.047) v.046 の顔の向きガイド 3D 版（head3d）を revert：開発時間の兼ね合いで一旦削除。データ形式・ボタン・描画ディスパッチ・pointer/wheel 処理・UPDATE_LOG 記載を含む v.046 の変更をすべて取り消し。将来再開する場合は git 履歴（コミット 74e44fd）を参照。
