...
- タスクタブのリロード復元をさらに堅牢化：boot 時と renderTabs で「未解決 ID（暗号化解錠前・R2 非同期取り込み中など `getNode` が null を返す状態）」を `state.openTasks` から破壊的に削除しないよう変更。表示は解決できた ID のみで行い、内部リストは温存する。close ボタンと「すべて閉じる」も表示中タブのみを対象にして、別プロジェクトで温存中のタブへ意図せず飛ばないよう修正。
- 左サイドバーのカテゴリ／ショットの並び順を作成順 → 名前順（自然順ソート）に変更。ショットナンバー（"SC001"→"SC010"→"SC100"）で上から並ぶようになる。
- ショット直下の工程の並び順を「工程設定（カテゴリ／ショットで並べた順）」に追従するよう変更。従来はハードコードの `stageRankCmp`（LAY→ANIM→アルファベット）に固定されていたが、`section.stages` 配列の順序を優先。工程設定に無い工程は末尾に回して名前順、`stages` 未設定のプロジェクトは従来どおり `stageRankCmp` にフォールバック。
- mp4 動画アップロード直後に「ブラウザで再生できないコーデックの可能性があります」オーバーレイが誤表示される不具合を修正。src 差し替え／R2 URL 解決のタイミングで一過性の `error.code=4` が発火して即オーバーレイが出る構造だったのを、(a) src 未設定時の error は無視、(b) `loadedmetadata`/`canplay` が来たら誤爆と判定して既存 badge も除去、(c) error 発火後 ~1.2 秒の猶予を挟んで、その間にロード成功があれば表示を取り消す方式に変更。実際にコーデック不対応の場合は従来通り表示される。
- 上記に加えて根本対処：`uploadVersion` 内で元 File／サムネ Blob からその場で blob URL を作って `storage.urlCache` に投入する。直後の `renderBody` は同期でこの URL を `<video>.src` に設定するため、R2 からの再ダウンロードや src 差し替えの隙間が発生せず、アップロード完了時点で「URL が確定→変わらない」状態になる。一瞬のオーバーレイフラッシュも根本的に起きない。
- さらにレビュー画面（openReview / FB オーバーレイ）でも同様の対処：video 要素生成直後に `storage.urlCache` を同期参照して src を即設定。ローカル（フォルダ）運用でも「アップロード直後にアノテ画面を開くとオーバーレイが一瞬出る」ケースを根絶。async 経路も `media.src` が既に確定していれば早期 return して二重 src 代入を避ける。
- 動画サムネイルのキャプチャ位置を「1 秒地点（または動画中盤）」から「1 フレーム目（動画の頭）」に変更。`genThumb` / `genThumbFromUrl` の両方で `currentTime` を 0.001 に固定（0 だと seeked が発火しないブラウザ対策）。

---

## 反映済み・パッチノート記載なし（Beta 反映済み・PATCH_NOTES.md 未記載）

- タスクページのタブがブラウザリロードで消えていた不具合を修正（ハッシュ復元経路でも `openTasks` / `expanded` を localStorage から復元、URL の対象がタスクなら自動でタブに追加）。※ beta v0.0.3 のまま据え置きで反映（バージョン非バンプ）。
