...
- (dev v2026.07.23.022) REEL の下部クリップ帯に各クリップのサムネイル画像を表示するよう追加。各 `.clip` ブロックの上部に `.cth` サムネ枠（`v.thumb`／`node.thumbnail` 由来）を差し込み、`.track` の min-height を 96px に拡張。
- (dev v2026.07.23.023) REEL のクリップサムネを `background-size:cover` → `contain` に変更し、サムネ全体が枠に収まるように修正（フレームからはみ出さない）。
- (dev v2026.07.24.001) 「クリア」ボタンの動作を変更：現在フレームのアノテのみクリアするようにした（他フレームの未送信内容は保持）。通常アノテウィンドウと REEL の両方に適用。
- (dev v2026.07.24.001) 矢印↑↓ でアノテ／コメントが付いているフレーム間をジャンプできるショートカットを追加。通常アノテウィンドウは版内のフレーム、REEL は全クリップを通したグローバルフレームで動作。アノテ・コメント両方の付いたフレームが対象。
- (dev v2026.07.24.001) REEL の再生範囲（IN/OUT）修正：再生中に IN/OUT を設定した場合に監視ループを即時起動、クリップ末尾で range OUT を跨がないよう `ended` ハンドラで判定、ループ時にクリップを跨ぐ範囲でも再生が継続するよう明示的に play() を再発火。
- (dev v2026.07.24.001) 動画の初回再生時に「音が出ないときはタブを右クリック →「サイトのミュートを解除」でオンにできます」のトースト（セッション内 1 回だけ）を表示。ブラウザ側からタブミュートの解除は不可能なため、案内のみ。
- (dev v2026.07.24.002) ↑↓ アノテ間ジャンプの方向を逆に：↑=次の（フレーム番号が大きい方向）、↓=前の に変更。
- (dev v2026.07.24.002) REEL の再生範囲を tick() ループでも冗長に判定するよう追加。単体の rAF ウォッチャーが取りこぼしても毎フレーム tick でチェックするためより堅牢に。
- (dev v2026.07.24.002) タブミュート案内トーストの表示ロジックを修正：sessionStorage → JS 変数（ページ読み込み毎 1 回）、REEL ポップアップから呼び出した際は REEL ウィンドウ側にも同時に toast を追加、REEL 用 `toastIn(doc,msg)` ヘルパを新設。
