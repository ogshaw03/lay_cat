...
- (dev v2026.08.07.040) ショットタブ：タイル直接クリックで作業ページ（タブ）に追加する動作に変更
  - 従来：`openReviewDrawer(...)` で右スライドを開く
  - 変更後：`go(id)` で直接タブに追加（アノテーション作業ページを新規タブとして開く）
  - 選択モード中（`state.shotSel` が非空）／Ctrl・Meta クリックの挙動は従来通り（選択トグル）
  - 縦並び行（`mkRow`）の挙動は今回変更せず（要望が「タイル」のみだったため）
