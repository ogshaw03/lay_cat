...
- 「チェック依頼」通知がチェック担当者に届かない不具合を修正。真の原因は「タスクページからの `＋ 動画` 提出は `DB.submits` に記録を残さず `uploadVersion` だけを呼ぶ経路」なのに対し、通知走査は `DB.submits` しか舐めていなかったため、この提出経路では通知が生成されなかった点。修正内容：(1) 走査に「各タスクの新版アップロード」パスを追加。`n.reviewer` が自分の review ノードを走査し、`ver.submitId` が無い（＝サブミット未経由）版で他者アップロードのものをチェック依頼として通知。キーは `rq:v:...` で、既存の `rq:...`（サブミット経由）と衝突しない。(2) 前段の submits 経由と合わせて、重複抑止は `DB.notifications.rqKey` で判定する新方式に統一。旧 `localStorage seen` にキーだけ残ってしまう事故で恒久静音化していたケースも解消。(3) サブミット作成時に `byEmail`（認証メール）を保存し「自分の提出をスキップ」判定をメールで厳密化。担当者名の比較も大文字小文字無視化。
- 動画プレイヤーに音量／ミュート切替を追加。FB オーバーレイのコントロールバーに 🔊/🔇 トグル＋音量スライダ、タスクページタイルのプレイヤーに 🔊/🔇 トグルを追加。動画要素は明示的に `muted=false`／`volume=1.0` で起動するようにして、副作用で暗黙にミュートされていた場合でも音が出る状態にする。ミュート・音量は `localStorage` にセッション間で保存。
