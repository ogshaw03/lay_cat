...
### AE 相当のタイムリマップ（レビュー画面）

**背景**：レビュー中に「特定区間を任意速度で再生／フリーズフレーム／逆再生／リタイム」で細かく確認したいユースケース。

**技術的な可否**：可能。LayCAT は既に EXR 連番用に自前スクラブ／再生ループを持つので、映像再生を setInterval + `video.currentTime` 制御で置き換えれば任意の時間マッピングが実装できる。

**必要な機能**
- タイムリマップキーフレームの UI（timeline に打点、値編集、削除）
- キーフレーム補間（Linear / Hold / Bezier）
- 再生時に「表示時刻 → ソース時刻」を毎フレーム計算して seek
- スクラブも同じマッピングを通す
- キーフレームカーブエディタ（表示用と編集用）

**コスト目安**
- 最小版（Linear 補間のみ・UI 最低限）：3〜5 日
- AE 相当（Hold/Bezier・カーブエディタ・アノテ連動）：2〜3 週間

**難所（重い順）**
1. **アノテーションとの整合性**：アノテはソースフレームに紐付いているため、リマップ後は「表示フレーム t → ソース f」のマッピングを通して描画位置・タイムラインマーカー・ゴースト・pending を全部変換する必要
2. **ネイティブ `<video>` の限界**：`playbackRate` は 0.25〜4x 程度、負値（逆再生）はブラウザによって不安定。フリーズ・逆再生の確実な実装は requestVideoFrameCallback + 手動 seek のフレーム制御が必要
3. **フレーム精度**：`video.currentTime` の seek 精度がキーフレーム間で滑らかに見えるか要検証（H.264 は 1F ズレが起きる）

**代替案（軽量）**
- 「特定区間だけスロー再生」用途なら、再生速度スライダー（0.25x / 0.5x / 1x / 2x）＋セクションループ（IN/OUT 間繰返し）＋フレームステップの拡充で 90% のユースケースはカバーできる

**判断ポイント**
- 本格的なタイミング調整をレビューで確認したい → タイムリマップ実装の価値あり（動画 DCC 的な位置づけ）
- 単に細かく確認したいだけ → 軽量代替案で十分
