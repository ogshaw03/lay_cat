...
- 「チェック依頼」通知がチェック担当者に届かない不具合を修正。原因は `sb.by === currentUser()`（表示名だけの比較）で「自分の提出をスキップ」していた判定が、同姓同名メンバーが居ると他人の提出まで誤ってスキップさせていた（名前しか一致していなくても "自分の提出" として扱う）。サブミット作成時に `byEmail`（認証メール）を保存し、通知走査ではメールで厳密比較するよう修正（未認証・旧サブミットは従来通り名前フォールバック）。あわせて担当者名の比較を大文字小文字無視化。
