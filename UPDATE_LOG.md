...
- **【2026-07-22 Beta 反映 まとめ】** 以下の一連の修正を beta v0.0.3 のまま据え置きで反映（バージョン非バンプ・パッチノート未記載）：
  - タスクタブのリロード復元を堅牢化：ハッシュ復元経路でも `openTasks` / `expanded` を localStorage から復元、URL の対象がタスクなら自動でタブに追加。加えて未解決 ID（暗号化解錠前・R2 非同期取り込み中）を `state.openTasks` から破壊的に削除しないよう変更。close ボタンと「すべて閉じる」も表示中タブのみを対象。
