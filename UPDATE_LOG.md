...
- (dev v2026.09.20.048) **アノテ窓／REEL のクリアボタン：送信済みアノテがある時は拒否＋案内トースト**：
  - 現在フレームに「送信済みの描画付きコメント」（アノテ窓：noteEmbedOnly でない・drawing 有／REEL：reel_emb_ でない・drawing 有）が 1 件でもあれば、クリアを実行せず「送信済みのアノテーションはクリア出来ません、コメント欄から削除してください」トーストを 5 秒表示。
  - 未送信 pending や embed 専用ノートしか無い場合は従来通りクリア可能。
  - APP_VERSION：2026.09.20.047 → 2026.09.20.048
...
- (dev v2026.09.20.046) **サブミット提出モーダルから工程選択 UI を撤去（Simple B 統一）**：
  - `submitStagesOf(shot)` を「常に単一を返す」形に統一。
    ...
    - レガシー shot section：`currentStage` が指す review、無ければ先頭
  - 結果、`mkRow` は常に `isSingle` 分岐に入り、工程チップは出ず「行クリックで選択／解除」に統一。
  - 文言修正：
    - 「提出するショット・工程を選択」→「提出するショットを選択」
    - 「ショットと工程を選択してください」→「提出するショットを選択してください」
    - 「このショット工程へのコメント」→「このショットへのコメント」
  - APP_VERSION：2026.09.20.045 → 2026.09.20.046

- (dev v2026.09.20.045) **動画削除がガードで拒否される問題を修正**：
  - v.033 の shot.json 書込みガード（versions が失われる save を拒否）が、ユーザーによる意図的な版削除（node._vtomb に tombstone 記録済み）も一律拒否してしまい、動画・画像の削除ができない状態だった。
  - ガード判定を「old にあって new に無い版」ごとに `newN._vtomb` を照合するように改良。tombstone に含まれていれば意図的な削除として通す。tombstone に無いもの（＝事故で消えた版）だけ拒否する。
  - APP_VERSION：2026.09.20.044 → 2026.09.20.045

- (dev v2026.09.20.044) **動画アップロード時のメッセージをメッセージ欄から除外**：
  - v.043 で log-side（フレームコメント / FB）とアノテ窓に「📤 アップロード時」として表示するようにしたが、ショットページのメッセージストリームにも従来通り「メッセージ」として表示されていて重複していた。
  - `renderReviewBody` の stream 構築で、`version.uploadCommentId` が指す comment はメッセージストリームから除外。
  - 結果：動画アップロード時のコメントは「フレームコメント / FB」とアノテ窓にだけ表示される。
  - APP_VERSION：2026.09.20.043 → 2026.09.20.044

- (dev v2026.09.20.043) **動画アップロード時のコメントを「フレームコメント / FB」パネルにも表示**：
  - v.022 でアノテ窓のコメント欄一番上に「📤 アップロード時 — 」として表示していた処理をヘルパ `_uploadCommentBlock(v,node,container)` に切り出し。
  - ショットページの動画ログ item 右側の log-side（フレームコメント / FB）先頭にも同じブロックを差し込むように。動画・画像どちらでも表示。
  - 空メッセージ判定（`log-empty-note` の表示）はアップロードコメントの存在も考慮するように更新。
  - **書込境界**：表示ロジックのみ。データは既存 `version.uploadCommentId` / `node.comments` を参照するだけで書き込み無し。
  - APP_VERSION：2026.09.20.042 → 2026.09.20.043

- (dev v2026.09.20.042) **アノテ窓／REEL のコメント欄に画像添付機能を追加**：
  - コメント入力欄に「＋」ボタンを追加（ファイル選択ダイアログを開く）。
  - textarea へのクリップボード貼り付け（画像）も対応。
  - 添付画像は最大辺 800px にリサイズして dataURL 化し、`note.attachImg` として保存。
  - サムネプレビュー＋× ボタンで送信前に取り消し可能。
  - 送信時：`buildFrameNotes`（アノテ窓）または `reelSendCur`（REEL）が `attachImg` を新 note に付与。
  - `noteBlock` で `n.attachImg` を表示（クリックで別タブに拡大）。
  - 画像だけの添付でも送信可（テキスト・描画が無くても note を作成）。
  - **書込境界**：既存の shot.json への通常書込のみ（新しい note に attachImg フィールドが追加されるだけ）。
  - APP_VERSION：2026.09.20.041 → 2026.09.20.042

- (dev v2026.09.20.041) **ショットページ追加モーダル：冗長な「初期工程」プルダウンを撤去**：
  - 工程セット編集リストの下にあった「初期工程（任意）」プルダウンは冗長なので削除。
  - 代わりに保存時、工程セットに 1 個以上あれば先頭を新ショットの `currentStage` に自動設定。
  - APP_VERSION：2026.09.20.040 → 2026.09.20.041

- (dev v2026.09.20.040) **ショットページ追加モーダルに工程セット編集 UI と初期工程プルダウンを実装**：
  - v.038 で置いた「初期工程プルダウンのみ」の簡易 UI では、工程セットが空のプロジェクトで工程を追加する手段が無かった。
  - 追加した UI（ショットページ選択時のみ表示）：
    - **テンプレート適用**：`root.stageTemplates` から選んで「テンプレートを適用」ボタンで反映。
    - **工程セット編集リスト**：各工程行に「色チップ・名前入力・削除」、末尾に「＋ 工程を追加」。
    - **初期工程プルダウン**：作成する review の `currentStage` を選択（未設定も可）。
    - 工程セットが編集されると初期工程プルダウンが自動再構築される。
  - **書込境界**：ショットページ作成時、工程セットに差分があれば `root.stages` を更新（laycat.project.json のみ）。shot.json は各新ショットの `kind:'shot'` と `currentStage` のみ書き込み。
  - APP_VERSION：2026.09.20.039 → 2026.09.20.040
...
- (dev v2026.09.20.027) **Simple B 変換で root.stages が空だと工程プルダウンが空になる問題を修正**：
  ...
  - APP_VERSION：2026.09.20.026 → 2026.09.20.027

- (dev v2026.09.20.026) **ホットフィックス：v.025 で「LayCAT が真っ白」になる不具合を修正**：
  - v.025 で `convertLegacyToSimpleB` 関数を追加した際、次のブロックのコメント冒頭 `/* ============` を巻き込んで削除してしまい、コメント本文が JS として解釈されてシンタックスエラーになっていた。
  - コメント冒頭を復元。node の構文チェックが通ることを確認。
  - APP_VERSION：2026.09.20.025 → 2026.09.20.026
...
- (dev v2026.09.20.024) **サブミット提出ピッカーの工程選択を Simple B では非表示に**：
  ...
  - APP_VERSION：2026.09.20.023 → 2026.09.20.024

- (dev v2026.09.20.023) **優先（急ぎ）アップロード → チェック担当者にモーダル通知**：
  - 動画アップロードモーダルに「🚨 優先（急ぎでチェック依頼）」チェックボックスを追加。ON で `version.priority=true` を焼く。
  - `scanMentionsForMe` 実行時（起動時／通知ベル開時／ポーリング時）に `showPriorityModalIfAny` を呼び、自分がチェック担当・自分以外がアップロードした priority=true 版を集めて赤枠モーダルで一覧表示。
  - モーダルの各行クリックで該当版のレビュー窓（アノテ窓）が即座に開く。「すべて確認済みにする」で全件 ack、「あとで確認」で閉じるだけ（ack しない＝次スキャンで再表示）。
  - ack は localStorage `laycat_prioAck`（vid の Set）に保存。1 度確認したものは再表示しない。
  - **書込境界**：shot.json 内の `versions[].priority` フラグ追加のみ。既存フィールドや notes 配列は触らない。
  - APP_VERSION：2026.09.20.022 → 2026.09.20.023

- (dev v2026.09.20.022) **アップロード時のコメントをアノテ窓のコメント欄一番上に表示**：
  - 動画・EXR アップロード時に入力されたコメント（node.comments の kind:'upload'）の id を `version.uploadCommentId` として版に紐付け。
  - アノテ窓の `renderNotes` で、まず版に紐付いたアップロードコメントを「📤 アップロード時 — 作成者 (日時)」ヘッダで最上部に表示。水色左ボーダー＋薄い水色グラデ背景で他コメントと差別化。
  - **書込境界**：新規プロパティ `version.uploadCommentId`（shot.json 内 versions[].uploadCommentId）を追加するのみ。ノート配列や既存フィールドは触らない。既存アップロードには id が無いので表示されず、新規アップロードから自動反映。
  - APP_VERSION：2026.09.20.021 → 2026.09.20.022

- (dev v2026.09.20.021) **マイルストーンの装飾を無彩色化・枠を撤去**：
  - LayCAT ヘッダーの milestoneBar から背景グラデーションと枠を撤去。文字だけの控えめな表示に。
  - 📌 に `filter:grayscale(1) contrast(.85)` を適用して無彩色化。日付色も青系から `--text2/--text3` に変更。
  - APP_VERSION：2026.09.20.020 → 2026.09.20.021

- (dev v2026.09.20.020) **LayCAT ヘッダーはマイルストーンを閲覧専用表示に**：
  - 作業者は PMB を基本使わないので本体ヘッダーにマイルストーン表示を復活。ただし「追加／編集は PMB」の役割分担に合わせて閲覧のみ（クリックで全件一覧ポップアップ）。
  - マイルストーン未登録時はバー自体を非表示（作業者を煩わせない）。
  - PMB からの更新を取り込むため 30 秒ごとに再読込。
  - APP_VERSION：2026.09.20.019 → 2026.09.20.020
...
- (dev v2026.09.20.018) **マイルストーン機能を追加（LayCAT 本体）**：
  ...
  - **書込境界**：プロジェクトフォルダ直下の `milestones.json` のみ（`storage.loadMilestones` / `storage.saveMilestones` を新設）。shot.json / laycat.project.json は一切触らない。
  - **権限**：プロジェクトメンバーなら誰でも編集可（アプリ側の役職 運営/管理者/メンバーとは無関係）。
  - APP_VERSION：2026.09.20.017 → 2026.09.20.018
...
- (dev v2026.09.20.011) **各クリップの 1F 目のアノテが焼き込まれない不具合を修正**：
  ...
  - v.010 のデバッグカウンタで「M が途中から増える／1F 目のアノテだけ抜ける」ことが確認できたための対処。
  - APP_VERSION：2026.09.20.010 → 2026.09.20.011

- (dev v2026.09.20.010) **書き出し進捗にアノテ焼き込み数を表示（不具合切り分け用）**：
  - 「アノテ込みなのに焼き込まれない」という報告の切り分けのため、書き出し中の進捗テキストに `アノテ描画: N` を追加。N が増えない場合は notes/frame マッチが取れていないこと、増えるのに動画に載らない場合は canvas→MediaRecorder のパイプラインの問題と判別できる。
  - APP_VERSION：2026.09.20.009 → 2026.09.20.010

- (dev v2026.09.20.009) **書き出しオプションのアノテ有無をチェックボックスからトグルボタンに変更（アノテ込みでも無しで出る不具合対応）**：
  - v.008 のチェックボックス方式は iframe（REEL 窓）内で初期状態と視認性が噛み合わず、「アノテ込み」を選んだつもりでも実際は off で送られていたケースがあった。
  - 明示的な 2 択セグメントボタン（アノテ込み／アノテ無し）に置き換え、選択中は青くハイライト。状態が視覚的に明白になったので誤操作の余地なし。既定は「アノテ込み」。
  - APP_VERSION：2026.09.20.008 → 2026.09.20.009

- (dev v2026.09.20.008) **REEL の書き出しにアノテ有無オプションを追加**：
  - Reel ファイルメニューの「⬇ アノテ込みで書き出し」を「⬇ 書き出し…」に改名し、押すと書き出しオプションの小窓を表示。
  - 小窓内に「アノテーションを含める」チェックボックスを配置（既定 ON）。書き出し開始／キャンセルボタン付き。
  - `exportAnnotatedReelClips` と `exportAnnotated` に `annotations` フラグを追加。false のとき描画時のアノテ焼き込みをスキップし、ファイル名も `_annotated` を外す。
  - 進捗表示タイトルも「アノテ込み／アノテ無し」を明示。
  - APP_VERSION：2026.09.20.007 → 2026.09.20.008

- (dev v2026.09.20.007) **REEL の 👁 ボタンでアノテが非表示にならない不具合を修正**：
  - **原因**：REEL の `drawAnno` は `RV.eye` を「保存済みノート」の描画にだけゲートしていたが、pending strokes／drafts／shape draft／head・mannequin プレビュー等はゲート外で常に描画されていたため、👁 OFF にしても消えないケースがあった。
  - **修正**：`drawAnno` 冒頭で `!RV.eye` なら clear のみで早期リターンに変更。ボタンテキストも `👁` ↔ `—` で状態が視認できるように（アノテ窓と同挙動）。
  - APP_VERSION：2026.09.20.006 → 2026.09.20.007

- (dev v2026.09.20.006) **REEL アノテ込み書き出しのクリップ境界カクつきをダブルバッファで解消**：
  - **原因**：`exportAnnotated` はクリップ境界で `video.src` を差し替え `onloadeddata` を待ってから `play()` していたため、その間 canvas が前クリップの最終フレームで固まり、音声も一瞬途切れて出力動画にカクつきが記録されていた。
  - **修正**：`<video>` を 2 つ用意し交互にアクティブ運用。次クリップは常にバックグラウンドで先読みしておき、境界では pause→play をほぼ待ち時間ゼロで切替。旧アクティブには次の次のクリップを先読み投入。
  - AudioContext には両 `<video>` に対する MediaElementSource を貼って dest に接続（inactive 側は pause なので同時再生にはならない）。
  - rvfc は世代番号で管理し、swap 後は旧 gen コールバックを破棄。
  - **書込境界**：書き出し I/O のみ。データファイルへの書き込みなし。
  - APP_VERSION：2026.09.20.005 → 2026.09.20.006

- (dev v2026.09.20.005) **前後カットでREELに送る：ショット祖先探索の不具合を修正（3 カット並ばず 1 カットしか送られなかった）**：
  - 旧実装は `_isShotLikeNode(review)` が true を返すため、レガシー multi-stage 構造で `node=review`（stage レビュー）にとどまり、`shotsOf(root)` が返す実ショット（section 側）と id 不一致で `sendToReel` フォールバックに落ちていた。
  - 修正：ショット祖先探索を「node から辿って `shotsOf(root)` に含まれるノードを見つける」ロジックに変更。Simple B（review が直接ショット）／レガシー（section がショット）どちらも正しく解決。
  - APP_VERSION：2026.09.20.004 → 2026.09.20.005

- (dev v2026.09.20.004) **前後カットでREELに送る：v.003 を revert し中央カットは「今見ている版」に戻す**：
  - ユーザー確認により v.003 の変更は誤解に基づくものと判明したため revert。
  - 挙動を v.002 に戻す：中央（現ショット）＝ 今ユーザーが見ている版、前後 ±1 ＝ 最新動画。
  - APP_VERSION：2026.09.20.003 → 2026.09.20.004

- (dev v2026.09.20.003) **前後カットでREELに送る：現在カットも「最新動画」で送るように統一**：
  - v.002 は中央カットだけ「ユーザーが見ている版」をそのまま送っていたが、要望に合わせて 3 カット全て「そのショットの最新動画」で並べるように変更。
  - APP_VERSION：2026.09.20.002 → 2026.09.20.003

- (dev v2026.09.20.002) **ショットページの「REELに送る」を「前後カットでREELに送る」に変更**：
  - REEL は本来「前後カットとの繋がりを確認する」ための機能であり、単一カットだけ送っても価値が薄いという指摘対応。
  - 動画版アクション行のボタンラベルを `▶ 前後カットでREELに送る` に、動作を `sendToReelWithNeighbors` に切替。
  - 挙動：現在編集中の版はそのまま、ショット順（`shotsOf(root)`）で前後 ±1 カットは最新動画で REEL に並べる。前後が無ければその分だけ省略。
  - **書込境界**：REEL 追加のみ（既存 `reelAddClip` 経由）。shot.json 等の変更なし。
  - APP_VERSION：2026.09.20.001 → 2026.09.20.002

- (dev v2026.09.20.001) **進捗タブ：工程グラフで工程をクリックするとステータスグラフがその工程のショットで絞り込まれるように**：
  - `state.pmPick` が `stage::…` のとき、ステータスグラフのデータを `_filterProgressByStage` で該当工程配下のショットに絞り込む（複数 EP 表示なら該当 EP のみに適用）。
  - ステータス列の見出しに「※ 「工程名」の工程で絞り込み中」バッジと「解除」ボタンを追加。
  - **書込境界**：表示のみ。データ書き込みなし。
  - APP_VERSION：2026.09.19.055 → 2026.09.20.001

- (dev v2026.09.19.055) **進捗タブのフォルダ選択をショットタブから分離**：
  - **背景**：進捗タブのフォルダプルダウンで選択したフォルダが `cur.shotsParentIds` に書き込まれ、その結果ショットタブや提出ピッカーも同じフォルダに絞り込まれてしまっていた。ユーザーからの「別の表示機能なので分けて」要望対応。
  - **修正**：進捗タブ専用に `cur.pmProgressFolderId`（単一フォルダ id）を新設し、プルダウンの読み書きをそちらに切替。旧 `shotsParentIds` には一切書き込まない。
  - **レガシー継承**：`pmProgressFolderId` 未設定で `shotsParentIds` が単一 id のときのみ、進捗タブは読み取り専用でその値をフォールバック使用し UX を維持（データ書き換えなし）。
  - **書込境界**：laycat.project.json の root ノードに `pmProgressFolderId` を追加／削除するのみ。`shotsParentIds` は変更しない。shot.json 系は一切触らない。
  - APP_VERSION：2026.09.19.054 → 2026.09.19.055

- (dev v2026.09.19.054) **進捗タブ「プロジェクト直下」でフォルダがショット扱いになっていた不具合を修正**：
  - **原因**：`progressData` がショット列挙に `childrenOf(pid)` を使っており、直下しか見ていなかった。プロジェクト直下配下に EP フォルダ等があるとフォルダ自身がショットとして表に縦並びで出ていた。
  - **修正**：ショットタブ（`shotsOf`）と同じ再帰＋型フィルタ（`_isShotLikeNode`）に揃え、選んだフォルダ配下の実ショットのみを集める。純粋なフォルダは中を掘り、review／レガシー shot section のみを "ショット" とみなす。
  - **書込境界**：表示ロジックのみ。データ書き込みなし。
  - APP_VERSION：2026.09.19.053 → 2026.09.19.054

- (dev v2026.09.19.053) **タグ付けモーダルを本物のチップ UI に**：
  - v.052 は横長の行だったのを、丸みのあるピル型チップを flex-wrap で流し込むレイアウトに変更。
  - 付与済み＝白塗り／未付与＝ブラックアウト／一部＝白＋破線ボーダー＋ドット。ホバー時にわずかに浮き、チップ内右端に × アイコン（ホバー時のみ表示）で削除。ダブルクリックで名前変更。
  - APP_VERSION：2026.09.19.052 → 2026.09.19.053

- (dev v2026.09.19.052) **タグ付けモーダルの見た目をチェックボックスからチップ状に変更**：
  - `openBulkTagModal` のタグ一覧をチェックボックス方式からチップ状の行に変更。
  - 付与済み（all）＝白背景・濃色文字、未付与（none）＝ブラックアウト（暗い背景・くすんだ文字）、複数ショット選択時の一部付与（some）＝白ベース＋破線ボーダー＋「一部」バッジ。
  - 行のどこをクリックしても toggle（編集／削除ボタンの上は除外）。ホバーで背景が薄く明るくなる。
  - APP_VERSION：2026.09.19.051 → 2026.09.19.052

- (dev v2026.09.19.051) **タグを担当フィルタバーの右に一覧表示（複数選択フィルタ）／v.050 のグルーピング切替は撤回してフォルダ表示に戻す**：
  - v.050 で入れた「フォルダ／タグ」グルーピング切替は撤回。フォルダとタグは同居前提のため、フォルダのセクション表示（v.047）は据え置き。
  - `assignFilterBar` の右側に `root.tags` を chip として一覧表示。クリックで toggle（複数選択）。選択したタグの少なくとも 1 つを持つショットだけを表示（OR マッチ）。
  - `shotFilterPass` にタグ判定を追加。担当フィルタとの併用は AND（担当かつタグ）。
  - 「フィルタ解除」ボタンはタグ選択もクリアするよう拡張。メンバー未登録でもタグがあればバー自体は表示。
  - **書込境界**：表示のみ。データ書き込みなし。
  - APP_VERSION：2026.09.19.050 → 2026.09.19.051

- (dev v2026.09.19.050) **ショットタブにタグでグルーピング切替を追加**：
  - ヘッダの「グリッド／縦並び」トグルの隣に「フォルダ／タグ」グルーピング切替を追加（`state.shotsGroupBy`、localStorage に保存）。
  - **タグモード**：`root.tags` の順にセクション化。複数タグ持ちのショットは各グループに重複表示。タグ未付与ショットは末尾「タグなし」（薄色）に。
  - **フォルダモード**：v.047 と同じ（親フォルダごとのセクション）。
  - **書込境界**：表示のみ。データ書き込みなし。
  - APP_VERSION：2026.09.19.049 → 2026.09.19.050

- (dev v2026.09.19.049) **ショットページの「使用アセット」下にタグ表示パネル追加**：
  - `buildShotTagsPane` を新設し、そのショットに付いているタグを chip で列挙（`#tagname`）。
  - 各 chip の × ボタンでそのショットからタグを外す（当該 shot.json の `tags` のみ更新）。
  - パネル右上の「＋ 編集」で `openBulkTagModal` をこのショット単独で開き、既存タグの付与／解除・新規タグ作成もその場で可能。
  - **書込境界**：shots/{sid}.json の `tags` フィールドのみ（× ボタンで外した場合）。モーダル経由なら v.048 と同じ書込境界。
  - APP_VERSION：2026.09.19.048 → 2026.09.19.049
...
- (dev v2026.09.19.047) **ショットタブをフォルダ単位でセクション分けして表示**：
  ...
  - **書込境界**：**表示のみの変更**。shot.json / status.json / laycat.project.json への書き込みは一切なし。
  - APP_VERSION：2026.09.19.046 → 2026.09.19.047

- (dev v2026.09.19.046) **ショットタブ・進捗タブがショットページ（Simple B review）に対応**：
  - **背景**：ショットタブは `shotsOf(root)` の結果を並べる仕様だが、`shotsOf` は「ショット親フォルダの直下子」を type 問わず返していた。そのため `root → EP folder → ショットページ` のようなネスト構造では EP フォルダ自体がショットとして並んで実物のショットページが出ない、逆に純粋なフォルダも "ショット" 扱いされる問題があった。
  - **修正**：`shotsOf` を再帰化＋型フィルタ。`_isShotLikeNode` で「review 型」or「レガシー shot section（子が全て review）」だけを "ショット" と判定し、それ以外の section（純粋なフォルダ）は中を掘る。ショットが多重にネストされていても正しく集約。
  - **影響**：ショットタブ・進捗タブ・工数タブ（`shotsOf` を使う全経路）で Simple B review が正しくショットとして表示される。既存のレガシー multi-stage 構造も無変更で動作継続。
  - APP_VERSION：2026.09.19.045 → 2026.09.19.046
...
- (dev v2026.09.19.044) **ショットページの工程プルダウンに工程設定の色を反映**：
  ...
  - **オプション各項目**：`<option style="color:xxx">` で工程色を反映（対応ブラウザで表示）。
  - Simple B・レガシー両モードとも同処理。
  - APP_VERSION：2026.09.19.043 → 2026.09.19.044

- (dev v2026.09.19.043) **左サイドバーでフォルダをクリックしても自動展開しないように**：`go(id)` が `pathOf(id)` 全体を `state.expanded` に追加していたので、フォルダを選ぶたびに自身も展開されていた。target 自身を除外して先祖のみ展開するよう修正。展開は「▶」トグルクリックで明示的に行う（Maya アウトライナ風）。APP_VERSION：2026.09.19.042 → 2026.09.19.043

- (dev v2026.09.19.042) **進捗タブのフォルダプルダウンからショットページ相当を除外**：
  - v.037 で追加したプルダウンは `type==='section'` で単純フィルタしていたが、レガシー multi-stage shot container（section 直下が全て review）も section 型なので混入していた。UX 上これはショットページ扱いなので、プルダウンには出さない。
  - `_isShotSection(n)` 判定を追加：`kids.length>=1 && kids.every(k=>k.type==='review')`。walk 側でこれを除外。
  - APP_VERSION：2026.09.19.041 → 2026.09.19.042
...
- (dev v2026.09.20.009) **マイルストーン縦線を水色に戻す／左サイドバーの凡例を削除**：
  - ガントのマイルストーン縦線を視認性優先で `#7ecbff` の破線 2px に（ラベルの装飾撤去はそのまま）。
  - 左サイドバーの「凡例」ブロック（予定・実績・作業中・マイルストーンのスウォッチ）を削除。
  - APP_VERSION：2026.09.20.008 → 2026.09.20.009

- (dev v2026.09.20.008) **ガント上のマイルストーンを無彩色化・枠を撤去**：
  - 縦線を青破線 2px → グレー破線 1px に。ラベルは青枠＋背景をやめ、テキストだけの表示に。
  - 📌 に grayscale フィルタ適用。編集ポップアップの機能はそのまま。
  - APP_VERSION：2026.09.20.007 → 2026.09.20.008
...
- (dev v2026.09.20.006) **マイルストーンをガントに縦線として描画**：
  ...
  - `drawGanttMilestones` ヘルパを追加し、`buildGantt` / `buildCustomGantt` の末尾で呼び出し。今日マーカーと同じ CSS 変数（`--shot-col-w`）で位置決めした縦破線＋テキストラベルを描画。
  - **書込境界**：pmboard 側は読取のみ。書き込みは LayCAT 本体のマイルストーン管理モーダルから。
  - APP_VERSION：2026.09.20.005 → 2026.09.20.006
